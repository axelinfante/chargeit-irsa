...
# ==============================
# PANTALLA PRINCIPAL + VALIDACIÓN (CAMBIO PRINCIPAL AQUÍ)
# ==============================
codigo_input = None
...
def validar_codigo(e):
    ...
    pantalla_layout([
        ft.ProgressRing(
                color=ft.Colors.WHITE,           # ← Color blanco
                stroke_width=8                   # ← Un poco más grueso para que se vea mejor
            ),
        ft.Text("Validando código...", size=26, weight=ft.FontWeight.BOLD, color="white")
    ])

    async def async_proceso():
        # 1. Validar que el código existe y es válido
        ...
        # 2. Marcar el código como utilizado en la API (REDENCIÓN)
        pantalla_layout([
            ft.ProgressRing(
                color=ft.Colors.WHITE,           # ← Color blanco
                stroke_width=8                   # ← Un poco más grueso para que se vea mejor
        ...
        redimido, error_redencion = await asyncio.to_thread(redimir_codigo_api, codigo_lower, VENDING_CODE)

        if not redimido:
            log.error(f"Error al redimir código {codigo_lower}: {error_redencion or 'Error desconocido'}")
            _mostrar_alert_firestore(
                "Error", 
                "Error al validar el código, solicite soporte en ¡Appa!",
                on_ok=pantalla_principal
            )
            return

        # 3. Si se redimió correctamente → Dispensar el producto
        pantalla_layout([
            ft.ProgressRing(
                color=ft.Colors.WHITE,           # ← Color blanco
                stroke_width=8                   # ← Un poco más grueso para que se vea mejor
...
def pantalla_reportes():
    ...
    def al_buscar(e):
        ...
        async def cargar():
            ...
            # ... (el resto del código de reportes se mantiene igual)
